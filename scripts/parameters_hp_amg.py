...
def run_instance(counter, simulation, n_refinements, k, type, policy, max_level, cs):
    ...
    datastore["CoarseGridSolverType"] = cs
        ...
        for k in ks:
            ...
            if options.hmg:
                ...
                for policy in ["DefaultPolicy", "CellWeightPolicy-1.0", "CellWeightPolicy-1.5", "CellWeightPolicy-2.0", "CellWeightPolicy-2.5", "CellWeightPolicy-3.0", "FirstChildPolicy"]:
                    run_instance(counter, simulation, n_refinements, k, "HPMG-local",  policy, 0, "amg")
                    counter = counter + 1;

            # global coarsening
            for policy in ["DefaultPolicy", "CellWeightPolicy-1.0", "CellWeightPolicy-1.5", "CellWeightPolicy-2.0", "CellWeightPolicy-2.5", "CellWeightPolicy-3.0", "FirstChildPolicy"]:
                run_instance(counter, simulation, n_refinements, k, "HMG-global" if options.hmg else "HPMG",  policy, 0, "amg")
                counter = counter + 1;

            # HP-multigrid + AMG
            if options.hpmg:
                for cs in ["amg", "amg_petsc"]:
                    run_instance(counter, simulation, n_refinements, k, "HPMG",  "DefaultPolicy", n_refinements + 1, cs)
                    counter = counter + 1;
